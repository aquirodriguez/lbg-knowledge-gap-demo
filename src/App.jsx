...
function GlassCard({ children, className = "" }) {
  return (
    <div className={`rounded-[28px] border border-white/70 bg-white/48 shadow-glass backdrop-blur-2xl ${className}`}>
      {children}
    </div>
  );
}
...
function QuestionIcon() {
  return (
    <svg aria-hidden="true" className="h-5 w-5" fill="none" viewBox="0 0 24 24">
      <path d="M8.7 9.2a3.5 3.5 0 1 1 5.9 2.5c-.9.8-1.8 1.3-1.8 2.7" stroke="currentColor" strokeLinecap="round" strokeWidth="1.9" />
      <path d="M12 18h.01" stroke="currentColor" strokeLinecap="round" strokeWidth="2.4" />
      <path d="M5.5 4.9h13A2.5 2.5 0 0 1 21 7.4v9.2a2.5 2.5 0 0 1-2.5 2.5H8.8L4 21v-3.9a2.5 2.5 0 0 1-1-2V7.4a2.5 2.5 0 0 1 2.5-2.5Z" stroke="currentColor" strokeLinejoin="round" strokeWidth="1.6" />
    </svg>
  );
}
...
function QuestionCard({ controls }) {
  return (
    <motion.div
      animate={controls}
    ...
    >
      <GlassCard className="relative overflow-hidden border-[#fff8eb] p-5">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-balance text-lg font-bold leading-snug text-[#0d2254] sm:text-xl">{question}</p>
          </div>
        </div>
      </GlassCard>
    </motion.div>
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
      ...
      <GlassCard className="border-[#f0d897]/90 bg-[#fff8eb]/52 p-5">
        ...
        <p className="text-xl font-extrabold leading-tight text-[#0d2254]">Good question - this is worth adding.</p>
...
function ReviewStack({ active }) {
  return (
    <motion.div
      animate={active ? { opacity: 1, y: 0, scale: [1, 0.97, 1.02, 1] } : { opacity: 0.62, y: 10, scale: 0.98 }}
...
function KnowledgeGapFlow() {
  ...
  useEffect(() => {
    if (step === "start") cardControls.set({ x: 0, y: 0, scale: 1, opacity: 1, rotate: 0 });
  }, [cardControls, step]);
  ...
  async function playFlow() {
    ...
    setStep("move");
    await cardControls.start({ x: isDesktop ? 260 : 0, y: isDesktop ? 158 : 150, scale: 0.55, rotate: 2, opacity: 0.95 });
    await cardControls.start({ x: isDesktop ? 284 : 0, y: isDesktop ? 186 : 178, scale: 0.16, opacity: 0 });
    setStep("stack");
    await delay(760);
    setStep("seal");
    await delay(1250);
  ...
  return (
    <main className="relative z-10 mx-auto flex min-h-screen w-full max-w-7xl flex-col px-4 py-6 text-[#0d2254] sm:px-7 lg:px-9">
      ...
      <section className="mb-5 text-center">
        <h1 className="text-balance text-3xl font-extrabold uppercase tracking-[0.12em] text-[#10255a] sm:text-5xl">Knowledge Gap Flow</h1>
      </section>
      ...
      <div className="grid flex-1 gap-5 lg:grid-cols-[0.96fr_1.1fr_0.96fr] lg:items-stretch">
        <FlowStage number="1" title="Ask A Question" className="min-h-[390px]">
          <div className="space-y-4">
            ...
            <p className="text-sm leading-relaxed text-[#64708a]">The best available answer appears first, with approved-source boundaries kept clear.</p>
          </div>
        </FlowStage>
        ...
        <FlowStage number="2" title="Safe Answer">
          <div className="space-y-5">
            ...
            <AnimatePresence mode="wait">
              {finalVisible ? (
                <motion.div animate={{ opacity: 1, y: 0 }} className="text-center" initial={{ opacity: 0, y: 12 }} key="final">
                  <h2 className="text-balance text-3xl font-extrabold text-[#10255a]">We'll let you know when the answer is ready.</h2>
                  <p className="mx-auto mt-3 max-w-sm text-base leading-relaxed text-[#5c6780]">You'll see it in Updates.</p>
                  <div className="mt-6"><SoftButton variant="gold">Done</SoftButton></div>
                </motion.div>
              ) : thanksVisible ? (
                <motion.div animate={{ opacity: 1, y: 0 }} className="text-center" initial={{ opacity: 0, y: 12 }} key="thanks">
                  <GoldSeal show />
                  <h2 className="mt-6 text-3xl font-extrabold text-[#10255a]">Thank you, Aqui.</h2>
                  <p className="mx-auto mt-3 max-w-sm text-base leading-relaxed text-[#5c6780]">Your question may help the next coworker who asks.</p>
                </motion.div>
              ) : (
                <motion.div animate={{ opacity: 1, y: 0 }} className="flex justify-center" exit={{ opacity: 0, y: -8 }} initial={{ opacity: 0, y: 8 }} key="button">
                  <SoftButton disabled={running} onClick={playFlow}>Continue</SoftButton>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
        ...
        <FlowStage number="3" title="Question Captured" className="min-h-[390px]">
          <div className="relative grid h-full min-h-[300px] place-items-center overflow-visible">
            <motion.div animate={step === "capture" || step === "move" ? { opacity: 1, scale: 1.08 } : { opacity: 0, scale: 0.92 }} className="absolute h-64 w-64 rounded-full bg-[#edc454]/24 blur-3xl" />
            {step === "move" && <motion.div animate={{ opacity: [0, 1, 0] }} className="absolute left-12 top-14 h-40 w-52 rounded-full border-b-2 border-l-2 border-[#f2ca65]/60 blur-[1px]" initial={{ opacity: 0 }} />}
            <ReviewStack active={stackActive} />
            <div className="absolute bottom-1 left-1/2 w-full max-w-xs -translate-x-1/2 text-center">
              {sealVisible && !thanksVisible && !finalVisible ? <GoldSeal show /> : <p className="text-sm leading-relaxed text-[#5d6882]">We'll let you know when the answer is ready.</p>}
            </div>
          </div>
        </FlowStage>
      </div>
      ...
      <footer className="mt-5 rounded-[22px] border border-white/70 bg-white/40 px-5 py-4 shadow-[0_18px_48px_rgba(41,36,24,0.07)] backdrop-blur-xl">
        <p className="text-center text-sm font-semibold text-[#10255a] sm:text-base">
          ...
          <span className="block font-medium text-[#667088] sm:inline">Every question strengthens our knowledge and our team.</span>
        </p>
      </footer>
    </main>
  );
